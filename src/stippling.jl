...
linearized_sites(sites, sz_grid) =
...
function split_by_density!(
    ...
    for (ix, p) in enumerate(sites_combined)
        ...
        for (k, (grid, density_interpolator)) in
            enumerate(zip(grid_single, density_interpolators))
            #@show p k centroids_single[ix][k] areas_single[ix][k]
        ...
        elseif density_selected > threshold_high
            ...
            for _tmp in eachcol([Δmove .-Δmove])
                ...
                centroid_moved = min.(max.(1, centroid_moved), size(grid_combined))
